...
void TransForm::showImage(QLabel* ql,Mat image,bool scaled) //在指定组件上展示图片
{
...
void TransForm::on_show_clicked()  //展示图片变换过程
{
    ...
    shape_predictor sp;
    deserialize("./shape_predictor_68_face_landmarks.dat") >> sp; //人脸识别68个特征点检测数据库
    ...
    Mat begin;
    Aimage.convertTo(begin,CV_32F);
    normalize(begin, begin, 0, 1, NORM_MINMAX);
    ...
    for(double alpha= 0.2;alpha<1; alpha += 0.2)
    {
        ...
        normalize(imgMorph, imgMorph, 0, 1, NORM_MINMAX);
        //原图被转化为32F，需要归一化，因为imshow会自动对32F图像每个像素值乘以256
        results.push_back(imgMorph);
    }
    Mat end;
    Bimage.convertTo(end,CV_32F);
    normalize(end, end, 0, 1, NORM_MINMAX);
    ...
    for(size_t i=0;i<results.size();i++)
    {
        ...
        for(int j=0;j<Bimage.rows;j++)
        {
            ...
            {
                 result.at<Vec3f>(j,k)[0]=results[i].at<Vec3f>(j,k-offset)[0];
                 result.at<Vec3f>(j,k)[1]=results[i].at<Vec3f>(j,k-offset)[1];
                 result.at<Vec3f>(j,k)[2]=results[i].at<Vec3f>(j,k-offset)[2];
            }
        }
    }
    Mat nresult(200,1200,result.type());
    cv::resize(result,nresult,nresult.size(),0,0,INTER_LINEAR); //缩小
    namedWindow("变形过程",WINDOW_AUTOSIZE);
    imshow("变形过程",nresult);
    waitKey(0);
    return;
}
